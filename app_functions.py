...
def st_plots(data, obj):
    ...
    # trans_obj = np.array(his_obj)
    # shift_4_bc = 0
    # #boxcox can only be with positive values, but they can be shifted to positive
    # if np.any(trans_obj <= 0):
    #     mini = trans_obj.min()
    #     shift_4_bc = abs(mini) + 0.000001
    #     trans_obj = trans_obj + shift_4_bc
    # print(trans_obj)

    # #BOX COXXXXXXXXXX
    # pt = PowerTransformer(method='box-cox')
    # pt.fit(trans_obj.reshape(-1,1))
    #
    # trans_obj = pt.transform(trans_obj.reshape(-1,1))
    # trans_obj = trans_obj.flatten()

    fig_trans, ax2 = plt.subplots()
    # plt.gca().title.set_text("Y after boxcox")
    # ax2.hist(trans_obj, bins=20)
    ...
    # first compute the cumulative yeields for each random seed
    cu_yields = []
    for seed in range(len(df.columns)):
        rseed = df.iloc[:, seed]
        yields = []
        if minmaxnum == 'max':
            current_opt = np.max(np.array(flatten(rseed[0])))
        elif minmaxnum == 'min':
            current_opt = np.min(np.array(flatten(rseed[0])))
        for qr in range(len(rseed)):
            query = np.array(flatten(rseed[qr]))
    ...
    for i in range(0, number_seeds):
        fig2.add_trace(go.Scatter(x=xvalues, y=plot[:, i], mode="lines", line=dict(color='black', dash='dot',
                                                                                   width=4),
                                  name='Random Seed ' + str(i + 1),
                                  customdata=plot[:, i],
                                  hovertemplate='x:%{x}<br>y:%{y}<br>z:%{z}<br>target: %{customdata} <extra></extra> '))

    fig2.add_trace(go.Scatter(x=xvalues, y=averages, mode="lines+markers", line=dict(color='navy', width=4),
                              marker=dict(size=2, color='navy'), name='Average'))
    ...
    fig2.update_layout(xaxis={'title': 'Experiment', 'title_font_size': 15, 'tickfont_size': 15},
                       yaxis={'title': 'Cumulative Yield', 'title_font_size': 15, 'tickfont_size': 15},
                       title=title,
                       showlegend=False,
                       template=custom_template)
    ...
    return fig_his, fig_trans, fig, fig2, opt
